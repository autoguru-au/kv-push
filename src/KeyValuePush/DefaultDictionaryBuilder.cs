...
public class DefaultDictionaryBuilder : IDictionaryBuilder
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        foreach (var filePath in filePaths)
        {
                ...
                recurseIntoJsonFiles)
            {
                ...
                {
                    throw new InvalidOperationException($"Problem reading {filePath} as dictionary.");
                }
    ...
    public static void TryAdd(IDictionary<string, string> dict, string key, string value)
    {
        if (dict.TryGetValue(key, out var val))
        {
            ...
            throw new InvalidOperationException($"Duplicate key of '{key}' with a different value detected.");
        }
